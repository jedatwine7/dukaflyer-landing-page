...
// ============================================================
// 6. Existing dukaAi function (now with image + PDF support)
// ============================================================
const DUKA_AI_MAX_FILE_BYTES = 4 * 1024 * 1024; // 4MB raw file size cap
const DUKA_AI_IMAGE_TYPES = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
...
exports.dukaAi = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      const { mode, message, history, shopContext, file } = req.body || {};
      ...
      const hasFile = file && typeof file === 'object' && file.data && file.mimeType;
      ...
      // Validate + prep the attachment, if any, before touching Claude at all.
      let pdfExtractedText = null;
      let imageBlock = null;

      if (hasFile) {
        // Only DukaFlyer shop owners can attach files — protects against
        // anonymous visitors running up vision/PDF-parsing costs. Checked
        // server-side (not just hidden in the UI), via a real Firebase Auth
        // ID token plus a Firestore lookup, since a client-side-only check
        // is trivial to bypass by calling this endpoint directly.
        const authHeader = req.headers.authorization || '';
        const idToken = authHeader.startsWith('Bearer ') ? authHeader.slice(7) : null;
        if (!idToken) {
          res.status(401).json({ error: 'Please sign in to attach files.' });
          return;
        }
        let requesterUid;
        try {
        ...
        const ownerShopSnap = await db.collection('shops').doc(requesterUid).get();
        if (!ownerShopSnap.exists) {
          res.status(403).json({ error: 'Only DukaFlyer shop owners can attach files right now.' });
          return;
        }

        const approxBytes = Math.floor((file.data.length * 3) / 4); // base64 -> raw size estimate
        if (approxBytes > DUKA_AI_MAX_FILE_BYTES) {
          res.status(400).json({ error: 'File too large — please keep attachments under 4MB.' });
          return;
        }
        ...
        if (file.mimeType === 'application/pdf') {
          try {
            ...
            const fullText = await processPDF(buffer);
            pdfExtractedText = fullText.slice(0, 3000);
          } catch (e) {
            console.error('dukaAi PDF processing failed:', e);
            res.status(400).json({ error: "Couldn't read that PDF — it may be scanned/image-based or corrupted." });
            return;
          }
        } else if (DUKA_AI_IMAGE_TYPES.includes(file.mimeType)) {
          imageBlock = {
            type: 'image',
            source: { type: 'base64', media_type: file.mimeType, data: file.data }
          };
        } else {
          res.status(400).json({ error: 'Unsupported file type — send an image (JPEG/PNG/GIF/WEBP) or a PDF.' });
          return;
        }
      }
      ...
      console.log('duka_question', JSON.stringify({
        ...
        hasFile: !!hasFile,
        fileType: hasFile ? file.mimeType : null,
...
Use this real shop data to ground your advice specifically in their business (e.g. reference their actual products/category/region) instead of giving generic advice, whenever it's relevant to what they're asking.`;
      ...
      if (imageBlock) {
        systemPrompt += `\n\nThe user has attached an image in this message. Look at it directly and respond to what's actually in it (e.g. a product photo, a screenshot, a receipt) — don't guess or ask them to describe it themselves.`;
      }
      ...
      // Build the final user turn — plain text, or a multi-part message when
      // there's an attachment (Claude's vision input for images; extracted
      // text spliced in for PDFs, same approach as the WhatsApp path).
      let userText = (message || '').slice(0, 2000);
      if (pdfExtractedText) {
        userText = `[Attached PDF${file.name ? `: ${file.name}` : ''}]\n${pdfExtractedText}\n\n${userText ? `User message: ${userText}` : 'The user attached this PDF without additional comment — summarize it and ask what they need.'}`;
      }

      if (imageBlock) {
        messages.push({
          role: 'user',
          content: [imageBlock, { type: 'text', text: userText || 'What do you see in this image?' }]
        });
      } else {
...
// ============================================================
// 9. Duka Live — real-time shop activity
// ============================================================

// NOTE: notification-sending for new views is already handled by your
// existing `onNewShopView` function (europe-west1, 5-minute batching window)
// — that one's live in production and reads/writes the same
// liveActivityEnabled/fcmToken fields, so it doesn't need to change.
// This function ONLY keeps the daily view counter current, since
// onNewShopView doesn't write viewsToday and the dashboard's "views today"
// figure depends on it.
exports.incrementShopViewCounter = onDocumentCreated(
  { document: 'views/{viewId}', region: 'us-central1' },
  async (event) => {
    const view = event.data?.data();
    if (!view || !view.uid) return;
    try {
      await db.collection('shops').doc(view.uid).update({
        viewsToday: admin.firestore.FieldValue.increment(1)
      });
    } catch (err) {
      console.error('incrementShopViewCounter error:', err.message);
    }
  }
);

// Reset every shop's daily view counter just after midnight, Kampala time.
exports.resetDailyViews = onSchedule(
  { schedule: '5 0 * * *', region: 'us-central1', timeZone: 'Africa/Kampala' },
  async () => {
    const snapshot = await db.collection('shops').where('viewsToday', '>', 0).get();
    const batches = [];
    let batch = db.batch();
    let count = 0;
    snapshot.forEach((docSnap) => {
      batch.update(docSnap.ref, { viewsToday: 0 });
      count++;
      if (count % 450 === 0) {
    ...
    console.log(`resetDailyViews: cleared viewsToday for ${count} shop(s).`);
  }
);

// Sweep up stale "active viewer" presence docs every 5 minutes, in case a
// buyer's tab closed without ever ticking over its heartbeat naturally.
// Requires a collection-group index on presence.lastSeen (Firestore will
// prompt with a console link the first time this runs if it's missing).
exports.cleanupStalePresence = onSchedule(
  { schedule: 'every 5 minutes', region: 'us-central1' },
  async () => {
    const cutoff = new Date(Date.now() - 10 * 60 * 1000); // 10 min
    const stale = await db.collectionGroup('presence').where('lastSeen', '<', cutoff).get();
    if (stale.empty) return;
    const batch = db.batch();
    stale.forEach((docSnap) => batch.delete(docSnap.ref));
    await batch.commit();
    console.log(`cleanupStalePresence: removed ${stale.size} stale presence doc(s).`);
  }
);